...
const MobileBettingSlip = () => {
  ...
  useEffect(() => {
    // Prevent scrolling when mobile slip is open
    if (bettingSlip.isMobileOpen) {
  ...
  const handlePlaceBet = async () => {
    ...
    try {
      ...
      bettingSlip.setMobileOpen(false);
    } catch (error) {
      toast({
  ...
  return (
    <div className="fixed inset-0 bg-darkbg bg-opacity-90 z-50">
      <div className="absolute bottom-0 left-0 right-0 bg-darkbg2 rounded-t-xl max-h-[80vh] overflow-auto">
        <div className="p-4">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-lg font-condensed font-bold text-lighttext">Bet Slip</h2>
            <button 
              onClick={() => bettingSlip.setMobileOpen(false)}
...
export default MobileBettingSlip;
